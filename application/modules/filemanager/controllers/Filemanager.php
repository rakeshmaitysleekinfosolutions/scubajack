...
class Filemanager extends AdminController {
    ...
    public function index() {
		...
		if ($this->input->get('directory')) {
			$directory = rtrim(DIR_IMAGE . 'frontend/' . getSession('user_id'). str_replace('*', '', $this->input->get('directory')), '/');
		} else {
            $directory = DIR_IMAGE . 'frontend/' . getSession('user_id');
		}
		...
		if (substr(str_replace('\\', '/', realpath($directory) . '/' . $filter_name), 0, strlen(DIR_IMAGE . 'frontend/' . getSession('user_id'))) == str_replace('\\', '/', DIR_IMAGE . 'frontend/' . getSession('user_id'))) {
			// Get directories
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$this->data['images'][] = array(
					...
					'href'  => site_url('filemanager?directory=' . urlencode(substr($image, strlen(DIR_IMAGE . 'frontend/' . getSession('user_id').'/'))) . $url)
				);
			} else {
                $this->data['images'][] = array(
...
//        }
		...
		$this->data['parent'] = site_url('filemanager?'.$url);
		...
		$this->data['refresh'] = site_url('filemanager?' . $url);
	...
	public function upload() {
		...
		// Make sure we have the correct directory
		if ($this->input->get('directory')) {
			$directory = rtrim(DIR_IMAGE . 'frontend/' . getSession('user_id') . '/'. $this->input->get('directory'), '/');
		} else {
            $directory = DIR_IMAGE . 'frontend/' . getSession('user_id');
		}
        // Check its a directory
        if (!is_dir($directory) || substr(str_replace('\\', '/', realpath($directory)), 0, strlen(DIR_IMAGE . 'frontend')) != str_replace('\\', '/', DIR_IMAGE . 'frontend')) {
...
//		$this->setOutput(json_encode($this->json));
	...
	public function delete() {
		...
		foreach ($paths as $path) {
            ...
            //exit;
			if ($path == DIR_IMAGE . 'frontend' || substr(str_replace('\\', '/', realpath(DIR_IMAGE . $path)), 0, strlen(DIR_IMAGE . 'frontend')) != str_replace('\\', '/', DIR_IMAGE . 'frontend')) {
